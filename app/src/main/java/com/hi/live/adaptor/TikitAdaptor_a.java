...
import com.hi.live.activity.TickitDetailActivity_a;
import com.hi.live.databinding.ItemTikitBinding;
...
public class TikitAdaptor_a extends RecyclerView.Adapter<TikitAdaptor_a.TikitViewHolder> {
    ...
    private List<ComplainRoot.DataItem> data;

    public TikitAdaptor_a(List<ComplainRoot.DataItem> data) {
    ...
    public class TikitViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(int position) {
            ComplainRoot.DataItem tikit = data.get(position);
            ...
            if (tikit.isSolved()) {
                ...
                binding.status.setBackgroundTintList(ContextCompat.getColorStateList(context, R.color.cgreen2));
            }

            itemView.setOnClickListener(v -> {

                context.startActivity(new Intent(context, TickitDetailActivity_a.class).putExtra("tickit", new Gson().toJson(tikit)));
            });
        }
    }
}
